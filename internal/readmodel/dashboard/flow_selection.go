...
func SelectFlowSelection(selectedSnapshotID uint, hasSelectedSnapshot bool, pos store.PositionRecord, isOpen bool, gates []store.GateEventRecord) (FlowSelection, *store.GateEventRecord, bool) {
	anchor := ResolveFlowAnchor(pos, isOpen, gates)
	selection := FlowSelection{Anchor: anchor}
	gate := selectGateForAnchor(anchor, gates)
	if !hasSelectedSnapshot {
		return selection, gate, true
	}
	gate = findGateBySnapshotID(selectedSnapshotID, gates)
	if gate == nil {
		return FlowSelection{}, nil, false
	}
	selection.Anchor = FlowAnchor{
		Type:       "selected_round",
...
func ResolveFlowAnchor(pos store.PositionRecord, isOpen bool, gates []store.GateEventRecord) FlowAnchor {
	if isOpen {
		...
		return FlowAnchor{Type: "latest_round", SnapshotID: 0, Confidence: "low", Reason: "no_history_for_open_position"}
	}
	if latest, ok := latestGateSnapshotID(gates); ok {
