...
func (g *Generator) buildFiles(req Request) (GenerateResult, error) {
	...
	files = append(files, GeneratedFile{Path: "scripts/onboarding-start.mk", Content: renderOnboardingMakefile(req)})

	return GenerateResult{Files: files}, nil
}

func renderOnboardingMakefile(req Request) string {
	symbols := req.Symbols
	if len(symbols) == 0 {
