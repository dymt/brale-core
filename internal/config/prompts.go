...
const defaultAgentIndicatorPrompt = "" +
	...
	"\n" +
	"新增输入字段说明：\n" +
	"- stc: Schaff Trend Cycle (0-100)，用于补充趋势建立和衰退的相位信息。state=rising 表示趋势正在建立，state=falling 表示趋势正在衰退，state=flat 表示相位不明确。\n" +
	"\n" +
	"重要约束：\n" +
...
const defaultProviderIndicatorPrompt = "" +
	...
	"- 若输入包含 stc，当 stc.state=rising 且与其他指标方向一致时，可增强 momentum_expansion 判断；当 stc.state=falling 且冲突较多时，signal_tag 应偏向 momentum_weak 或 noise。\n" +
...
const defaultInPosIndicatorPrompt = "" +
	...
	"若输入包含 stc，可把 stc.state 从 rising 转为 falling 视为动量走弱的辅助证据。\n" +
...
const defaultAgentStructurePrompt = "" +
	...
	"- 若输入包含 supertrend，它表示当前趋势状态和动态 trailing level，可辅助判断 regime change 与结构强弱，但不替代 structure_points 和 key_levels。\n" +
	"\n" +
	"重要约束（防止行动泄漏）：\n" +
...
const defaultProviderStructurePrompt = "" +
	...
	"- 若输入包含 supertrend，当 supertrend.state 与 regime 判断一致时，可增强 clear_structure；当 supertrend.state 与原结构方向矛盾时，integrity 应更保守。\n" +
...
const defaultInPosStructurePrompt = "" +
	...
	"若输入包含 supertrend，state 翻转或价格跌破/突破 supertrend.level，可作为 threat_level 升级和倾向 tighten 的辅助证据。\n" +
...
const defaultRiskFlatInitPrompt = "" +
	...
	"- 若结构锚点摘要包含 supertrend，它是独立的 trailing 参考，不替代 nearest_below_entry / nearest_above_entry；supertrend.level 可辅助判断止损是否过紧或过松。\n" +
...
const defaultRiskTightenUpdatePrompt = "" +
	...
	"- 结构锚点摘要中的 nearest_below_entry / nearest_above_entry 是相对 entry 的方向中性锚点：direction=long 时通常分别更接近止损/止盈参考；direction=short 时通常分别更接近止盈/止损参考。\n" +
	"- 若结构锚点摘要包含 supertrend，其 level 可作为 trailing stop 收紧参考，但不替代基于 swing / EMA / order block 的止损逻辑。\n" +
